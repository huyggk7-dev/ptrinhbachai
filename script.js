...
function formatResult(numerator, denominator) {
    ...
    if (Number.isInteger(numerator) && Number.isInteger(denominator)) {
        ...
        return `${kq} (làm tròn 2 chữ số thập phân)`;
    }
}
...
document.getElementById('quadraticForm').addEventListener('submit', function(event) {
    ...
    if (delta > 0) {
        ...
        solutionDisplay.innerHTML = `Phương trình có 2 nghiệm phân biệt:<br>x = ${x1}<br>x = ${x2}`;
    } else if (delta === 0) {
        // Nghiệm kép: x = -b / 2a
        ...
        // Vô nghiệm
        solutionDisplay.textContent = "Phương trình vô nghiệm vì Delta < 0";
    }
});